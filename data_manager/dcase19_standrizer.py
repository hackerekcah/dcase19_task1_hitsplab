...
class DCASE19Standarizer:
    ...
    def create_scaler_h5(self):
        ...
        with h5py.File(self.scaler_h5, 'w') as f:
            for mode in ['train', 'test']:
                for device in ['a', 'b', 'c', 'p', 'A', 'bc', 'abc']:
    ...
    def load_dev_mean_subtracted(self, mode='train', device='a', norm_device='a'):
        """
        mean subtracted
        :param mode:
        :param device:
        ...
        data, label = self.data_manager.load_dev(mode=mode, devices=device)
        # Take Care!!! only scale using train mu and sigma
        mu, _ = self.load_scaler(mode='train', device=norm_device)
        ...
        print("[LOGGING] Mean subtracted using device {}".format(norm_device))
...
if __name__ == '__main__':
    ...
    ori_data_manager = Dcase19TaskbData()
    ori_standarizer = DCASE19Standarizer(data_manager=ori_data_manager)
